...
class EditPengawasanAliranScreen extends StatefulWidget {
  final Datum? data;
  const EditPengawasanAliranScreen({this.data, super.key});

  @override
  State<EditPengawasanAliranScreen> createState() => _EditPengawasanAliranScreenState();
}

class _EditPengawasanAliranScreenState extends State<EditPengawasanAliranScreen> {
  File? _ktpFilePath;
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isSuccess) {
          ...
          Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(builder: (context) => const ListPengawasanAgamaScreen()),
            (route) => false,
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${data.message}')));
