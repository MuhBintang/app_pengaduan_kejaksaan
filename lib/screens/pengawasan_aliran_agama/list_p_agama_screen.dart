...
import 'package:app_kejaksaan/screens/pengawasan_aliran_agama/detailAliranAgama_screen.dart';
import 'package:app_kejaksaan/screens/pengawasan_aliran_agama/edit_pengawasanAliran.dart';
import 'package:app_kejaksaan/screens/pengawasan_aliran_agama/p_agama_screen.dart';
...
class ListPengawasanAgamaScreen extends StatefulWidget {
  final Datum? data;
  const ListPengawasanAgamaScreen({this.data, super.key});

  @override
  State<ListPengawasanAgamaScreen> createState() => _ListPengawasanAgamaScreenState();
}

class _ListPengawasanAgamaScreenState extends State<ListPengawasanAgamaScreen> {
  ...
  Future<void> getSessionData() async {
    ...
    setState(() {
      ...
      kategoriUser = prefs.getString("kategori");
    });
    getPengaduan();
  }
  ...
  Future<List<Datum>?> getPengaduan() async {
    try {
      ...
      http.Response res = await http.get(Uri.parse('$url/getPengaduan.php?id_user=$idUser&kategori=$kategoriUser'));
      List<Datum> data = modelPengaduanFromJson(res.body).data ?? [];
      
      // Filter berdasarkan kategori "penyuluhan hukum"
      List<Datum> filteredData = data.where((datum) => datum.kategori == "pengawasan aliran dan kepercayaan").toList();
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => PengawasanAgamaScreen()),
          );
        },
                              ...
                              return GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => DetailPengawasanAliranScreen(pengaduan: data),
                                    ),
                                  );
                                },
                                child: Padding(
                                              ...
                                              IconButton(
                                                ...
                                                onPressed: () {
                                                  Navigator.push(
                                                      ...
                                                      MaterialPageRoute(
                                                          builder: (context) =>
                                                              EditPengawasanAliranScreen(data: data,))
                                                              );
                                                },
                                                icon: Icon(
                                                  Icons.edit,
