...
class EditKorupsiScreen extends StatefulWidget {
  final Datum data;
  const EditKorupsiScreen(this.data ,{super.key});

  @override
  State<EditKorupsiScreen> createState() => _EditKorupsiScreenState();
}

class _EditKorupsiScreenState extends State<EditKorupsiScreen> {
  File? _ktpFilePath;
  ...
  Future<void> getPengaduanData() async {
    try {
      final response = await http.get(Uri.parse('$url/getKorupsi.php?id=${widget.data}'));
      if (response.statusCode == 200) {
        ...
        if (data != null) {
          setState(() {
            ...
            _uraianLaporanController.text = data['uraian_laporan'] ?? '';
  ...
  Future<void> editPengaduan() async {
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('$url/edit_korupsi.php'));
