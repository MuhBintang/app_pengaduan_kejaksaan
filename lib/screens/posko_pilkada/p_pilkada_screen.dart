...
class PoskoPilkadaScreen extends StatefulWidget {
  const PoskoPilkadaScreen({super.key});

  @override
  State<PoskoPilkadaScreen> createState() => _PoskoPilkadaScreenState();
}

class _PoskoPilkadaScreenState extends State<PoskoPilkadaScreen> {
  File? _ktpFilePath;
  ...
  Future<void> addphkm() async {
    try {
      ...
      if (_ktpFilePath != null && _laporanFilePath != null) {
        ...
        request.fields['kategori'] = "posko pilkada";
        ...
        if (response.statusCode == 200) {
          ...
          if (data.isSuccess) {
            ...
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => const ListPoskoPilkadaScreen()),
              (route) => false,
            );
          } else {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${data.message}')));
